...
一套面向建筑施工企业的 **AI 投标视频自动化平台**。上传招标文件、施工组织设计、模型截图与项目素材，系统自动完成解说词拆解、画面生成、AI 配音、多分段视频合成与素材复用，最终导出 **16:9、1080P** 的工程投标视频。当前 AI 主链路为 **Kimi K3（文本结构化/多模态）+ Seedream 4.5（图生图）+ Seedance 2.0（图生视频）**。
...
- **AI 视频生成（Phase 6/7，Seedance）**：独立「AI 视频生成」页面（`/project/:projectId/ai-video`），图片驱动视频工作流——用户主动选择首帧/尾帧、选择 42 种建筑视频模板、填写独立视频提示词（不引用解说词）、生成独立视频素材。新建视频任务当前仅开放 Seedance（火山方舟 Ark）；MiniMax 适配器保留作历史兼容，但不会被当前页面调用。支持首尾帧（顺序固定 `[first_frame, last_frame]`）、时长/比例/分辨率/随机种子/声音开关（默认关闭）；每次生成保存完整参数快照可复现；结果版本可预览/下载/选为当前。AI 视频结果不绑定解说词分镜，只在「视频工程」中由视频分段选择素材；建筑强约束默认启用，冲突指令（增加楼层/改变轮廓/移动道路/替换主楼）阻止提交。
- **提示词大师与施工高级工作台**：Kimi K3 多模态读取首帧/尾帧或多参考图，输出可编辑的镜头提示词与结构化施工配方。快速生成页只使用普通镜头提示词；只有进入并确认应用施工工作台后，WBS、工程事实、状态转换、施工时间轴、摄影时间轴、空间锚点和验收清单才会投喂 Seedance。最终编译层统一控制时长、画面比例和分辨率；页面选择的分辨率是唯一输出规格，提示词中的「4K/超高清」等冲突描述会被清理。
...
| 提示词大师（首尾帧视觉理解） | `adapters/llm.py` | `kimi-k3`（默认，文本+图片输入）/ `disabled`(mock) |
| 图片生成/参考图渲染 | `adapters/image.py` | `seedream`（默认）/ `minimax` / `openai` / `disabled`(mock) |
| 图生视频/首尾帧视频 | `adapters/video.py` | `seedance`（新任务唯一通道）/ `disabled`(mock) |
