...
微影支持多个账号共同参与同一个投标项目。项目访问不再只看创建者：项目拥有独立的成员体系，每个成员在项目中扮演一个角色，所有权限均由后端逐接口强制校验（非成员访问返回 404，成员越权返回 403），前端按钮状态只是体验层提示。

### 项目角色与权限

| 角色 | 定位 | 主要权限 | 不能做 |
| --- | --- | --- | --- |
| 项目所有者 owner | 项目第一责任人 | 全部权限：成员管理、角色调整、所有权转移、审核策略、删除项目、查看审计、带理由的管理覆盖 | 项目必须始终至少有一名 owner，最后一个 owner 不能退出、被移除或降级 |
| 投标负责人 bid_manager | 组织投标工作 | 编辑项目与资料、核对工程信息、编辑解说词与分镜、创建/分派待办、提交审核、正式导出 | 不能管理成员、变更审核策略、删除项目、查看审计 |
| 技术编辑 technical_editor | 核对工程事实 | 上传与阅读资料、核对工程参数与评分点、评论、创建待办、提交技术内容审核 | 不能编辑分镜与媒体、管理成员、正式导出 |
| 视频编辑 media_editor | 制作媒体内容 | 编辑解说词与分镜、画面制作、AI 视频、配音字幕、视频工程、提交媒体审核 | 不能核对工程事实（fact.edit）、管理成员、正式导出 |
| 审核人 reviewer | 独立审核 | 只读全部内容、发表评论与修改意见、批准/驳回/要求修改、把审核意见转为待办 | 不能编辑业务内容、不能批准自己提交的内容 |
| 只读成员 viewer | 查看进度 | 查看项目、资料、工程信息、分镜、素材与导出记录 | 不能编辑、评论、提交审核、创建待办 |

完整的角色-权限点矩阵可在「协作与审核 → 项目成员 → 角色权限说明」中查看（接口 `GET /api/v1/roles`）。

### 邀请与成员管理

项目所有者在「协作与审核 → 项目成员」中通过邮箱邀请成员。邀请令牌 7 天过期、只能使用一次，数据库只保存令牌哈希，原始令牌仅在创建或重发时显示一次；没有邮件服务时可直接复制邀请链接发给对方。邀请只能由对应邮箱的已登录账号接受。所有者可以修改成员角色、移除成员、撤销/重发邀请，也可以把所有权转移给任一有效成员（原所有者自动转为投标负责人，全过程记录审计）。平台账号被停用后，其项目成员关系保留但自动转为「已停用」，无法继续操作。

### 评论与待办

评论可挂接到工程信息、评分点、单个分镜、画面版本、AI 视频版本、配音版本、视频分段和导出任务，支持回复、解决与重新打开；所有目标都校验属于当前项目，跨项目 ID 一律 404。审核人标记的「阻断级」意见会阻止正式导出。任何评论都可以一键转为待办，待办可分派给项目成员、设置优先级与到期时间，状态变化会通知负责人与创建者并写入审计。

### 审核流程与批准失效

工程信息（批量）、分镜文稿（整份或单个分镜）和视频工程都可以提交审核。每次提交都会绑定当时的内容版本（revision + 快照哈希 + 内容快照），审核人可以在「快照对比」中看到提交时内容与当前内容的差异。

审核状态转换：

```
draft（未提交）→ in_review（审核中）
in_review → approved（批准）
in_review → changes_requested（要求修改，必须填写原因）
changes_requested → in_review（修改后重新提交，旧请求自动失效）
approved → approved_but_changed（批准后内容被再次修改，原批准自动失效）
approved_but_changed → in_review（重新提交）
```

要求修改的意见会自动沉淀为阻断级评论，并通知提交人。批准后内容再次被修改时，提交人与批准人都会收到「已批准内容发生变更」通知。审核人默认不能审核自己提交的内容；项目所有者在必要时应填写理由进行带审计的管理覆盖。

### 审核策略与正式导出门禁

每个项目可在「编辑项目」中配置审核策略（需要 owner 权限）：

- `disabled`：不启用协作审核（历史项目升级后的可选状态）；
- `recommended`（默认）：未完成审核时预检给出警告，但不阻断导出；
- `required`：正式导出必须满足——关键工程信息已批准、当前分镜文稿已批准、当前视频工程已批准、不存在未解决的阻断级审核意见。

演示导出（demo）始终可用，但当审核未完成时，预检面板与导出报告中会醒目标记「未完成正式审核」。正式导出权限（export.formal，仅 owner/bid_manager）与审核批准权限（review.decide，仅 reviewer/owner）分别检查。

### 并发编辑保护

项目、工程参数、分镜、视频工程和视频分段都带有 `revision` 乐观锁。编辑请求携带 `base_revision`，若服务器版本已经变化则返回 409，前端会弹出「内容刚被其他成员修改」对话框，允许你加载最新版本，或保留页面上的未提交编辑供复制比较——不会静默覆盖他人保存的内容。

### 通知与审计

邀请、待办分派与状态变化、审核请求与决定、评论提及、已批准内容变更都会产生站内通知（右上角铃铛）。成员变更、邀请、角色调整、所有权转移、评论、待办、审核决定、管理覆盖和超级管理员绕行访问全部写入项目审计记录（「协作与审核 → 审计记录」，仅 owner 可见），完整回答“谁在什么时候做了什么”。审计不记录完整敏感内容、API Key 或邀请原始令牌。
